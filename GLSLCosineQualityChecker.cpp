...
void GLSLCosineQualityChecker::loadShaders()
{
    ...
    if(!program->addShaderFromSourceCode(QOpenGLShader::Vertex, 1+R"(
#version 330
in vec4 vertex;
void main() { gl_Position=vertex; }
)"))
    {
       QMessageBox::critical(nullptr, QObject::tr("Shader compile failure"),
                             QObject::tr("Failed to compile %1:\n%2").arg("GLSL cosine quality check vertex shader")
                                                                     .arg(program->log()));
    }

    if(!program->addShaderFromSourceCode(QOpenGLShader::Fragment, 1+R"(
#version 330
...
void main()
{
    ...
    {
        QMessageBox::critical(nullptr, QObject::tr("Error compiling shader"),
                              QObject::tr("Failed to compile %1:\n%2").arg("GLSL cosine quality check fragment shader")
                                                                      .arg(program->log()));
    }
    if(!program->link())
    {
        QMessageBox::critical(nullptr, QObject::tr("Error linking shader program"),
                              QObject::tr("Failed to link %1:\n%2").arg("GLSL cosine quality check shader program")
                                                                   .arg(program->log()));
    }
}
